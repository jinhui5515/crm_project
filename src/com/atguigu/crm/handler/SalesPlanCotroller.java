...
/*		List<SalesChance> content = page.getContent();
	...
	public String savePlan(SalesPlan plan,RedirectAttributes attributes){
		try {
			salesChanceService.savePlan(plan);
			attributes.addFlashAttribute("message", "成功加入一个计划！");
			
		} catch (Exception e) {}
		Long id = plan.getChance().getId();
		return "redirect:make?id="+id;
	}
	@RequestMapping(value="/edit")
	public void edit(@RequestParam("id") Long id, @RequestParam("todo") String todo,HttpServletResponse response) throws IOException{
		
		
		String data = "0";
		PrintWriter writer = response.getWriter();
		if(id != null){
			SalesPlan plan = salesChanceService.getSalesPlanById(id);
			plan.setTodo(todo);
			salesChanceService.updateSalesPlan(plan);
			data = "1";
		}
		writer.write(data);
	}
	...
	public void saveResult(@RequestParam(value = "result")String result, 
			@RequestParam(value = "id")Integer id,HttpServletResponse response,
			Map<String,Object> map) throws IOException{
		
		SalesPlan salesPlan = salesChanceService.getPlanById(id);
		salesPlan.setResult(result);
		map.put("salesPlan", salesPlan);
		map.put("result", result);
		map.put("id", id);
		String data = "0";
		PrintWriter writer = response.getWriter();
		if(result != null){
			salesChanceService.resultUpdate(map);
			data = "1";
		}
		writer.write(data);
	}
